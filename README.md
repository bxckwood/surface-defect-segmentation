...
В этих запусках U-Net с BCE + Dice получила более высокий validation IoU и меньше пропусков, чем U-Net с BCE, при большем числе ложных тревог. ResNet18 U-Net показала наилучший validation IoU среди трёх моделей. При пороге 0.7 вместо 0.5 validation IoU вырос с 0.499 до 0.502. На test есть удачные выделения, неполные маски, пропущенные дефекты и крупные ложные выделения на чистых снимках.

## Ограничения

Фиксированный resize меняет масштаб дефектов. Один split и один seed не показывают разброс между запусками. Эксперимент с ResNet18 одновременно меняет архитектуру encoder и инициализацию, поэтому сравнение не доказывает, что улучшение вызвано конкретно предобучением. Порог 0.7 оказался лучшим только в небольшой сетке порогов на этой validation выборке; разница с 0.5 мала. В приведённом сравнении модель и порог выбраны на validation, а показатели test описывают выбранный вариант.
